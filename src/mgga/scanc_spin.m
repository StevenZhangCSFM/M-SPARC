function [ec,vc,v2c,v3c] = scanc_spin(rho,sigma,tau)
% @file    scanc_spin.m
...
% ==============================================================================================
 ...
 % v2c has 3 columns: [D(\epsilon_c(n)*n)/D(|grad n|) / |grad n|, 0, 0]
 % v3c has 2 columns: [D(\epsilon_c(n)*n)/D tau, D(\epsilon_c(n)*n)/D tau]
    v2c = zeros(size(rho,1),3);
    ...
    [ec, vc, Vc2, Vc3] = correlationSCAN(rho(:, 1), s, alpha, zeta, DsDn, DsDDn, DalphaDnup, DalphaDndn, DalphaDDn, DalphaDtau, DzetaDnup, DzetaDndn);
    v2c(:,1) = Vc2./normDrho(:, 1);
    v3c = [Vc3, Vc3];
end

function [s, alpha, zeta, DsDn, DsDDn, DalphaDnup, DalphaDndn, DalphaDDn, DalphaDtau, DzetaDnup, DzetaDndn] = basicMGSGAvariables(rho, normDrho, tau)
